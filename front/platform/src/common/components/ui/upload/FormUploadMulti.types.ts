...
/**
 * React Hook Form용 UploadMulti 컴포넌트 Props
 * Controller를 내부에 감추고 간결한 API를 제공합니다.
 */
export interface FormUploadMultiProps<TFieldValues extends FieldValues = FieldValues>
  extends Omit<UploadMultiProps, 'value' | 'onChange' | 'error'> {
  /** 필드 이름 (필수) */
