...
/**
 * 시도지사 추천요청 및 감정평가법인 입력 섹션
 * 조건부: selectedCheck true 시 감정평가법인·금액·의뢰/회신 공문 표시
 */
export function EvaluationCorporationInput() {
  const { control, watch, setValue } = useFormContext<ApplicationFormData>()

  const selectedCheck = useWatch({ control, name: 'evaluation.selectedCheck' })
  const amounts = watch('evaluation.amounts')
  ...
  const averageAmount = useMemo(() => {
    const a = Number(amounts?.amountA) || 0
    const c = Number(amounts?.amountC) || 0
    if (hasGovernorRecommendation && amounts?.amountB != null) {
      const b = Number(amounts.amountB) || 0
      return Math.round((a + b + c) / 3)
    }
    return a && c ? Math.round((a + c) / 2) : 0
  }, [amounts?.amountA, amounts?.amountB, amounts?.amountC, hasGovernorRecommendation])
