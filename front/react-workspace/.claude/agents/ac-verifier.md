...
- 경로 주석이 없는 코드 블록은 `## 시그니처` 섹션의 서브섹션명(API 서비스 / 훅 / 컴포넌트 Props / Store 액션)을 참고해 아래 기본 경로 패턴으로 glob 탐색한다.
...
- 테스트 이름(`"should ... when ..."`)이 `## 테스트 시나리오`의 시나리오 설명문과 일치하며 AC 의도를 반영하는가?
...
| 항목 | 규칙 |
|------|------|
| 파일 수정 | 금지 — 어떤 파일도 생성하거나 수정하지 않는다 |
| 구현 코드 생성 | 금지 |
| 테스트 추가 | 금지 |
