...
> CLAUDE.md의 JOOQ 규칙(`J{Pascal}.{SNAKE_UPPER}`, `{Pascal}Entity`)을 따른다.
> 기존 Repository 파일을 참고할 때는 `find datasource/{module-name}/src/main/java -name "*Repository*.java" | head -3`으로 후보를 탐색한다.
...
- Helper 생성 기준 (두 경로):
  - **경로 A** — `issue-{N}.md`의 `## 시그니처 → ### Helper` 섹션에 명시된 Helper
    → 반드시 구현한다 (200줄 기준 무관 — `tdd-red-be`가 이미 테스트를 작성했음)
  - **경로 B** — 시그니처에 없는 신규 Helper
    → Service 200줄 초과 또는 private 5개 이상일 때만 생성
    → 미충족 시 `/tdd-refactor-be`에서 추출

### 구현 순서

**경로 A — 시그니처에 Helper가 있는 경우:**
```
1. Helper  — 시그니처 구현 (Repository 주입·트랜잭션 금지)
2. Repository — 메서드 작성 (JOOQ DSL)
3. Service — Helper 주입 후 메서드 작성
4. Controller — 엔드포인트 완성: @PostMapping/@GetMapping, @Valid, 적절한 ResponseEntity 상태코드
               + @Tag(클래스), @Operation(메서드) — api/platform/CLAUDE.md 필수 컨벤션
```

**경로 B — Helper가 없는 경우 (기본):**
```
1. Repository — 메서드 작성 (JOOQ DSL)
2. Service — 메서드 작성: 검증 → repository 호출 → toResponse()
3. Controller — 엔드포인트 완성: @PostMapping/@GetMapping, @Valid, 적절한 ResponseEntity 상태코드
               + @Tag(클래스), @Operation(메서드) — api/platform/CLAUDE.md 필수 컨벤션
```

> **Reader가 필요한 경우**: Reader는 Helper(`@Component`)가 아닌 **@Service 레이어 컴포넌트**입니다.
> Repository 주입과 `@PlatformTransactional(readOnly = true)`가 허용됩니다.
> 이름 예시: `NoticeListReader`, `BoardContentReader`
...
| 상황 | 위치 |
|------|------|
| 변환 메서드가 1개 이상 | Service `private toResponse()` 메서드 유지 — `/tdd-refactor-be`에서 추출 여부 판단 |
| 변환 불필요 (예: DELETE 204, 단순 ID 반환) | `toResponse()` 작성하지 않음 — `ResponseEntity.noContent().build()` 또는 원시 타입 직접 반환 |

> Green 단계에서는 Converter 추출을 하지 않는다. 추출 여부는 Refactor 단계에서 판단한다.
...
| Helper | 시그니처 명시 시 즉시 구현 (경로 A). 미명시 시 Service 200줄 초과·private 5개 이상일 때만 생성 (경로 B) |
