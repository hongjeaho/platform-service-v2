...
안 C: CQRS-lite (조회/명령 분리)
  - 조회: Controller → {Domain}ReadService (@Service, Reader 주입) → {Domain}SummaryReader (@Service, Repository 주입 허용) + Cache
  - 명령: Controller → {Domain}WriteService (@Service) → [{Domain}Validator + Repository]
  - Reader는 Helper가 아닌 @Service 레이어 컴포넌트: Repository 주입과 @PlatformTransactional(readOnly=true) 허용
  - readOnly=true / readOnly=false 트랜잭션을 ReadService / WriteService로 완전 분리
