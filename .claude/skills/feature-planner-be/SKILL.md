...
description: |
  백엔드(Spring Boot) 아이디어를 개발 가능한 이슈 단위로 변환하는 기획 워크플로우 스킬.
  /feature-planner-be 명령어로 진입. Git 브랜치명(feature/xxx)을 자동 파싱해 feature-path를 설정하고,
  api/* 디렉토리를 스캔해 module-name(api/platform, api/admin 등)을 결정한다.
  api/* + datasource/* 두 모듈에 걸친 기능 기획을 지원하며 모듈이 늘어나도 스킬 수정이 불필요하다.
  main·master·develop 등 보호 브랜치에서 실행 시 feature 브랜치 생성을 먼저 안내한다.
  사용자가 백엔드 기능을 기획하거나, "백엔드 기획", "API 설계", "Spring Boot PRD",
  "백엔드 이슈 분해", "API 이슈 분해", "백엔드 spec", "feature-planner-be",
  "백엔드 수직 슬라이싱", "백엔드 ADR", "Controller 설계", "Service 설계" 등을 언급하면
  반드시 이 스킬을 사용할 것.
...
> **레퍼런스 구현체**: `api/platform`의 `board` 도메인.
> 새 도메인을 설계할 때는 `board/controller`, `board/service`, `board/dto`를 먼저 확인하고,
> 그 패턴에서 벗어나는 결정은 반드시 ADR에 기록한다.
...
/feature-planner-be {feature-path} {설명}   ← 경로 + 설명
```
...
### Step 2.5: 모듈 감지 + 패키지 루트 탐색

브랜치 확인 직후 `api/` 디렉토리를 스캔해 module-name을 결정하고,
...
**단일 모듈:**
```
📦 API 모듈: platform (api/platform + datasource/platform)
   이 모듈로 진행합니다. 다르면 말씀해주세요.
```

**복수 모듈:**
```
📦 감지된 API 모듈: platform, admin
   어떤 모듈의 기능인가요?
   예) platform → api/platform + datasource/platform
       admin    → api/admin + datasource/admin
```
...
| 브랜치명 | feature-path | 문서 경로 (pkg-root=com/platform/api/platform 예시) |
|---------|-------------|--------------------------------|
| `feature/tag` | `tag` | `api/platform/src/main/java/{pkg-root}/tag/docs/` |
| `feature/notice/list` | `notice/list` | `api/platform/src/main/java/{pkg-root}/notice/list/docs/` |
| `feature/notice/list-search` | `notice/list-search` | `api/platform/src/main/java/{pkg-root}/notice/list-search/docs/` |
...
📦 패키지 루트: com/platform/api/platform
📁 추론된 경로: notice/list
   → 문서 경로: api/platform/src/main/java/com/platform/api/platform/notice/list/docs/
...
```
/feature-planner-be {feature-path}
/feature-planner-be {feature-path} {기능 설명}
```

| 입력 예시 | 의미 | 문서 경로 (module-name=platform) |
|----------|------|--------------------------------|
| `/feature-planner-be tag` | tag 단독 기능 | `api/platform/src/main/java/.../tag/docs/` |
| `/feature-planner-be notice/list` | notice 도메인의 list 기능 | `api/platform/src/main/java/.../notice/list/docs/` |
| `/feature-planner-be notice/list 페이지네이션 포함` | 설명 추가 | `api/platform/src/main/java/.../notice/list/docs/` |

**파싱 규칙:**
- 첫 토큰에 슬래시(`/`)가 있거나 영문+하이픈 조합 → `{feature-path}` 직접 지정
...
```
api/{module-name}/src/main/java/{pkg-root}/{feature-path}/docs/
```
...
feature-path 결정
  ...
  └─ 브랜치 추론 → git branch --show-current
    ...
    ↓
module-name 결정 (api/* 스캔 → 자동 선택 또는 질문)
    ↓
pkg-root 탐색 (*Application.java 위치로 자동 결정)
    ↓
[CONTEXT 저장: feature-path, module-name, api-module, ds-module, pkg-root, docs-root]
    ↓
spec.md 존재 확인
    ├─ 있음 → 내용 보여주고 "이 내용으로 진행할까요?" 확인 [GATE]
    └─ 없음
         ├─ 기능 설명 있음 → spec.md 자동 생성 후 확인 [GATE]
...
# 3. 패키지 루트 탐색 (module-name이 결정된 후 실행)
find api/{module-name}/src/main/java -name "*Application.java" -maxdepth 8 | head -1

# 4. spec.md 존재 여부 확인 (pkg-root, feature-path가 결정된 후 실행)
ls api/{module-name}/src/main/java/{pkg-root}/{feature-path}/docs/spec.md 2>/dev/null
```
...
인터뷰 전 탐색 대상:
- `api/{module-name}/{domain}/service/helper/` — Helper 컴포넌트
- `datasource/{module-name}/repository/`, `datasource/{module-name}/mapper/` — 데이터 접근
- `api/{module-name}/config/cache/CacheNames.java`, `CacheConfig.java` — 캐시
- `api/{module-name}/config/exception/GlobalExceptionHandler.java` — 예외 처리
...
도메인 전용 예외가 필요하면 위 두 클래스를 상속해 정의한다:
```java
public class {Domain}NotFoundException extends IllegalArgumentException {
    public {Domain}NotFoundException(Long id) {
        super("{도메인}을 찾을 수 없습니다. id=" + id);
    }
}
```
...
7. 기존 백엔드 패턴과의 일관성 — 이 기능의 비즈니스 로직이 기존 Helper
   ...
   board 도메인 패턴(Controller→Service→Repository)에서 벗어난다면 그 이유는?
...
## 개요
{기능 목적과 범위 — 한 단락. 어떤 REST 엔드포인트가 노출되고, 누가 호출하며,
어떤 비즈니스 문제를 해결하는가.}

## 사용자 스토리
- As a {역할: 일반 사용자 / 관리자 / 시스템},
  I want to {HTTP 행동: POST /notice / GET /notice/{id} / ...}
  so that {목적}

## API 명세

> 이 명세가 Orval이 생성하는 프론트엔드 TypeScript 타입의 기준이 된다.
> API 변경 시 프론트엔드에서 `pnpm orval` 재실행 필요 여부를 함께 명시한다.

| HTTP | URI | 인증 | 요청 DTO | 응답 DTO | Orval 재실행 |
|------|-----|-----|---------|---------|------------|
| POST | /api/public/{domain} | 불필요 | {Domain}CreateRequest | {Domain}Response | 필요 |
| GET | /api/public/{domain}/{id} | 불필요 | — | {Domain}Response | 필요 |
| GET | /api/public/{domain} | 불필요 | (query params) | List<{Domain}Response> | 필요 |
| PUT | /api/{domain}/{id} | JWT 필요 | {Domain}UpdateRequest | {Domain}Response | 필요 |
| DELETE | /api/{domain}/{id} | JWT 필요 | — | — (204) | 필요 |
...
| 모듈 | 변경 대상 |
|------|---------|
| `api/{module-name}` | Controller, Service, DTO, (Helper — 복잡도 시 추출) |
| `datasource/{module-name}` | Repository/Mapper, Flyway SQL (`flyway/V{yyyyMMddHHmmss}__...sql`) |

### Helper 추출 계획
> **Service가 200줄 초과 또는 private 메서드 5개 이상이면 Helper로 추출한다.**
> Helper에 Repository를 주입하거나 트랜잭션을 시작하는 것은 금지.
> DB 접근 포함 로직은 Service에서 Repository를 직접 호출한다.

| Helper 클래스 | 역할 접미사 | 추출 이유 / 해당없음 |
|-------------|-----------|-------------------|
| {DomainValidator} | Validator | {중복 체크 등 검증 로직 분리} / 해당없음 |

### 예외 처리 계획
> **Controller에 try-catch 작성 금지.** Service에서 던지면 GlobalExceptionHandler가 처리한다.

| 예외 상황 | 사용 예외 클래스 | HTTP |
|---------|---------------|-----|
| {엔티티} 없음 | `IllegalArgumentException` | 400 |
| 중복 {엔티티} | `IllegalStateException` | 409 |

### 캐시 계획
> **CacheNames 상수로만 참조. 문자열 하드코딩 금지.**
> 새 캐시 추가 순서: `CacheNames.java` 상수 추가 → `CacheConfig.java`에 `createXxxCache()` 추가 → 사용.
> 캐시가 불필요하면 "캐시 없음 — 이유: {이유}"로 명시한다.
...
> 스키마 변경 시: `datasource/{module-name}/flyway/V{yyyyMMddHHmmss}__....sql` 작성
> → `./gradlew :datasource:{module-name}:generateJooq` 실행 → JOOQ 클래스 자동 갱신.

| 접근 목적 | 방식 | Flyway 필요 여부 |
|---------|-----|----------------|
| {목록 조회} | JOOQ Repository | 불필요 (기존 테이블) |
| {단건 조회} | JOOQ Repository | 불필요 |
| {생성} | JOOQ Repository | 필요 (`V{ts}__create_{table}.sql`) |
...
코드베이스의 기존 패턴(board 도메인)을 먼저 탐색한 뒤, 최소 3가지 구현 안을 제안하고
...
```
안 A: 단순 CRUD (board 패턴 반복)
  - Controller → Service → JOOQ Repository
  - Helper 없음, 캐시 없음
  - 조회 메서드: @PlatformTransactional(readOnly=true)
  - 명령 메서드: @PlatformTransactional
  - 채택 조건: 비즈니스 규칙이 단순하고 조회 빈도가 낮은 경우

안 B: 검증 Helper 추출
  - Controller → Service → [{Domain}Validator + Repository]
  - 중복 체크·상태 전이 검증 로직을 Validator로 분리
  - 읽기 캐시(@Cacheable) 적용
  - 조회 메서드: @PlatformTransactional(readOnly=true)
  - 명령 메서드: @PlatformTransactional
  - 채택 조건: 비즈니스 규칙이 있고 Service 코드가 200줄 초과 예상

안 C: CQRS-lite (조회/명령 분리)
  - 조회: Controller → Service → {Domain}SummaryReader + Cache
  - 명령: Controller → Service → {Domain}Validator + Repository
  - readOnly=true / readOnly=false 트랜잭션 완전 분리 (클래스 레벨 @Transactional(readOnly=true) + 명령 메서드 오버라이드)
  - MyBatis로 복잡 집계 쿼리 처리
  - 채택 조건: 조회가 압도적으로 많거나 통계/집계가 복잡한 경우
```
...
- `api/{module-name}/src/main/java/{pkg-root}/{feature-path}/docs/prd.md`
...
수평 슬라이싱 ❌ (마지막 이슈까지 완료해야 API 호출 가능):

이슈 1: Flyway 마이그레이션만 작성  ← curl 불가
이슈 2: TagRepository만 구현        ← curl 불가
이슈 3: TagService만 구현           ← curl 불가
이슈 4: TagController만 구현        ← 이슈 4 이후에야 처음 curl 가능
→ 이슈 단위 독립 테스트 불가능.
```
...
- `api/{module-name}/src/main/java/{pkg-root}/{feature-path}/docs/issues.md`
...
/tdd-green-be {N}        → 테스트 통과 최소 구현 (Flyway·JOOQ·Repository → Service → Controller)
/tdd-refactor-be {N}     → 구조 개선 (Helper 추출, board 패턴 일관성) [GATE]
/ac-verifier-be {N}      → AC 충족 독립 검증 — 테스트 통과 ≠ AC 충족 [GATE]
/security-review-be {N}  → 보안 취약점·패턴 위반·코드 품질 점검 [GATE]
/create-pr               → git commit 안내 + PR 제목·본문 생성
```
