...
> **브랜치 컨벤션**
> ```
> feature/[{root}/]{feature-path}/{설명}
>          └─ 선택사항: api | common | datasource | batch
> ```
> - 마지막 세그먼트: 사람이 읽는 **설명 레이블** (한글·영문 무관) — Java 패키지 경로 아님
> - 마지막 세그먼트 앞까지: **실제 Java 패키지 경로 (feature-path)**
> - `/` : 계층 구분자 (도메인/하위도메인)
> - `-` : 같은 계층 내 단어 연결

| 브랜치명 | root | feature-path | 문서 경로 |
|---------|------|--------------|----------|
| `feature/users/이메일-인증` | api (기본값) | `users` | `api/{module}/.../users/docs/` |
| `feature/users/email/이메일-인증` | api (기본값) | `users/email` | `api/{module}/.../users/email/docs/` |
| `feature/api/users/이메일-인증` | api (명시) | `users` | `api/{module}/.../users/docs/` |
| `feature/common/core/email-send` | common | `core` | `common/core/.../core/docs/` |
| `feature/datasource/users/repo-add` | datasource | `users` | `datasource/{module}/.../users/docs/` |

**변환 알고리즘:**
1. `feature/` prefix 제거
2. 첫 번째 세그먼트가 예약 키워드(`api` | `common` | `datasource` | `batch`)이면 **root**로 추출, 아니면 root = `api` (기본값)
3. **마지막 세그먼트 제거** (설명 레이블)
4. 남은 세그먼트를 **feature-path**로 사용

**예시 변환:**
```
feature/users/이메일-인증
  ↓ feature/ 제거
users/이메일-인증
  ↓ 첫 세그먼트 "users" → 예약 키워드 아님 → root: api (기본값)
  ↓ 마지막 세그먼트 "이메일-인증" 제거
feature-path: users

feature/users/email/이메일-인증
  ↓ feature/ 제거 → users/email/이메일-인증
  ↓ "users" → 예약 키워드 아님 → root: api (기본값)
  ↓ 마지막 세그먼트 "이메일-인증" 제거
feature-path: users/email

feature/api/users/이메일-인증
  ↓ feature/ 제거 → api/users/이메일-인증
  ↓ "api" → 예약 키워드 → root: api
  ↓ 마지막 세그먼트 "이메일-인증" 제거
feature-path: users

feature/datasource/users/repo-add
  ↓ feature/ 제거 → datasource/users/repo-add
  ↓ "datasource" → 예약 키워드 → root: datasource
  ↓ 마지막 세그먼트 "repo-add" 제거
feature-path: users
```

### Step 3.5: feature-path 파일시스템 검증

feature-path 추출 후 실제 패키지 디렉토리가 존재하는지 검증한다.

```bash
# 1순위: feature-path 디렉토리 존재 확인
find {root}/{module-name}/src/main/java/{pkg-root}/{feature-path} -maxdepth 0 -type d 2>/dev/null

# 2순위: 없으면 상위 경로에서 후보 디렉토리 목록 출력
ls {root}/{module-name}/src/main/java/{pkg-root}/ 2>/dev/null
```
...
| 결과 | 처리 |
|------|------|
| feature-path 디렉토리 존재 | 정상 진행 (docs 폴더는 신규 생성) |
| feature-path 디렉토리 없음 | 후보 목록 출력 후 사용자에게 확인 요청 |

**feature-path 디렉토리 없을 때 출력:**

```
⚠️  추론된 feature-path 경로를 찾을 수 없습니다.

   브랜치:           feature/users/이메일-인증
   추론된 feature-path: users
   확인한 경로:      api/platform/src/main/java/com/platform/api/platform/users/
   ...
   계속 진행할 feature-path를 선택하거나 직접 입력해주세요.
   예) users
       auth
       /feature-planner-be api/users 이메일 인증 기능
```

### Step 4: 추론 결과 사용자에게 확인

```
🌿 현재 브랜치: feature/users/이메일-인증
📦 루트:       api (기본값)
📦 모듈:       platform (api/platform + datasource/platform)
📦 패키지 루트: com/platform/api/platform
📁 추론된 feature-path: users
   → 문서 경로: api/platform/src/main/java/com/platform/api/platform/users/docs/
...
[CONTEXT] root:         api
          ...
          docs-root:    api/platform/src/main/java/com/platform/api/platform/users/docs/
          branch:       feature/users/이메일-인증
```
...
| 입력 예시 | 의미 | 문서 경로 |
|----------|------|-----------|
| `/feature-planner-be api/users` | api 하위 users 도메인 | `api/{module}/.../users/docs/` |
| `/feature-planner-be api/users/email` | users 하위 email 서브도메인 | `api/{module}/.../users/email/docs/` |
| `/feature-planner-be common/core` | common/core 하위 | `common/core/.../core/docs/` |
| `/feature-planner-be api/users 이메일 인증 기능` | feature-path=users + 설명 추가 | `api/{module}/.../users/docs/` |
...
#    규칙: feature/ 제거 → 첫 세그먼트가 예약어(api|common|datasource|batch)면 root 추출
#          → 마지막 세그먼트 제거(설명 레이블) → 나머지가 feature-path
#    예: feature/users/이메일-인증        → root=api(기본), feature-path=users
#    예: feature/users/email/이메일-인증  → root=api(기본), feature-path=users/email
#    예: feature/api/users/이메일-인증    → root=api, feature-path=users
...
# 5. feature-path 디렉토리 존재 확인 (root, module-name, pkg-root, feature-path가 결정된 후 실행)
find {root}/{module-name}/src/main/java/{pkg-root}/{feature-path} -maxdepth 0 -type d 2>/dev/null
